...
fn main() -> Result<()> {
    ...
    match pki_chain::storage::get_storage_state(&app_config.clone())? {
        pki_chain::storage::StorageState::NotFound | pki_chain::storage::StorageState::Empty => {
            tracing::info!("Storage not ready. Initializing storage...");
            pki_chain::storage::get_storage_empty(app_config.clone())
                .inspect_err(|e| tracing::error!(error = %e, "Failed to open empty storage"))?
                .create_storage()
                .inspect_err(|e| tracing::error!(error = %e, "Failed to create storage"))?
                .initialize_storage()
                .inspect_err(|e| tracing::error!(error = %e, "Failed to initialize storage"))?;
            tracing::info!("Storage initialized successfully.");
        }
        pki_chain::storage::StorageState::Created => {
            tracing::info!("Storage created but not initialized. Initializing storage...");
            pki_chain::storage::get_storage_created(app_config.clone())
                .inspect_err(|e| tracing::error!(error = %e, "Failed to open created storage"))?
                .initialize_storage()
                .inspect_err(|e| tracing::error!(error = %e, "Failed to initialize storage"))?;
            tracing::info!("Storage initialized successfully.");
        }

        pki_chain::storage::StorageState::Initialized => {
            tracing::info!("Existing storage found and initialized. Starting socket server for adding first admin.");
        }
        pki_chain::storage::StorageState::Ready => {
            tracing::info!("Storage is ready.");
        }
        pki_chain::storage::StorageState::Inconsistent => {
            ...
            return Err(anyhow::anyhow!("Storage is in an inconsistent state"));
        }
    }
