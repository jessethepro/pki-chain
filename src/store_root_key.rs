...
pub fn get_root_ca_pfx(
    ...
    // Create iterator to find genesis block
    let mut iter = pfx_chain.iter()
        .map_err(|e| anyhow!("Failed to create blockchain iterator: {}", e))?;
    
    // Find the genesis block (last block in descending iterator)
    let mut genesis_data: Option<Vec<u8>> = None;
    
    while let Some((_uid, data)) = iter.next() {
        genesis_data = Some(data);
    }
    
    let encrypted_bytes = genesis_data
        .ok_or_else(|| anyhow!("No genesis block found in PFX blockchain"))?;
