...
/// Store an Intermediate CA certificate in the certificate blockchain
///
/// This function:
/// 1. Retrieves the Intermediate CA PFX from the PFX blockchain at the specified height
...
/// * `pfx_height` - Height of the Intermediate CA in the PFX blockchain
...
///
/// Returns an error if:
/// - No Intermediate CA PFX exists at the specified height in the PFX chain
...
///     &pfx_chain,
///     1, // Height in PFX chain
///     &cert_chain,
///     &hasher,
///     &app_key_store
/// )?;
/// ```
pub fn store_intermediate_ca_certificate<H: BlockHeaderHasher>(
    ...
    // Retrieve the Intermediate CA PFX from the PFX blockchain
    let intermediate_ca_pfx = get_intermediate_ca_pfx(pfx_chain, pfx_height, app_key_store)
        .map_err(|e| anyhow!("Failed to retrieve Intermediate CA PFX: {}", e))?;
    
    // Extract the certificate in DER format
    let cert_der = &intermediate_ca_pfx.certificate_der;
...
/// Retrieve an Intermediate CA certificate from the certificate blockchain by height
///
/// This function:
...
/// # Returns
///
/// * `Result<Vec<u8>>` - The Intermediate CA certificate DER bytes on success
///
/// # Errors
///
/// Returns an error if:
...
/// # Example
/// ```no_run
/// use pki_chain::get_intermediate_ca_certificate;
/// use libblockchainstor::BlockchainDb;
///
/// let cert_chain = BlockchainDb::open("../data/certificates")?;
///
/// let intermediate_ca_der = get_intermediate_ca_certificate(&cert_chain, 1, &app_key_store)?;
/// // Use intermediate_ca_der for certificate operations
/// ```
pub fn get_intermediate_ca_certificate(
    certificate_chain: &BlockchainDb,
    height: u32,
    app_key_store: &AppKeyStore,
) -> Result<Vec<u8>> {
